...
- terms.json: 上游英文字段的五语言对照表，分 bodyParts(9)/equipment(9)/muscles(31) 三个命名空间；翻译人写，完整性机器裁决。
...
- **词表与目录完整闭合**：`terms.json` 三个命名空间的键集必须与 `exercises.json` 实际用到的 body_part / equipment / target+muscle_group+secondary_muscles 完全相等，且每条都有五语言。少一条中文界面就漏出一个英文词，多一条说明目录已经漂移；两个方向都在 `fitness-gui.test.ts` 里裁决。
