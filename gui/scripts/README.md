...
- i18n.js: zh-CN/en/ja/fr/es 五语言界面文案、语言协商（精确→主语言→英文兜底）与 {n} 插值纯函数；训练计划禁用原因包含授权/数据修复动作，肌肉名仍归 data/muscle-regions.json。
...
- main.js: 语言协商与文案填充、本地资源加载、按 body-map.json 组装男女正背面人体 SVG、动作目录与计划 dialog 编排；禁用原因常驻并关联入口，unknown/hard-conflict/已提交待刷新状态始终复用 frozen ids，只有 all-absent 证据后的可编辑状态才允许显式丢弃。
